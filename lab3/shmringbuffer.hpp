...
class ShmRingBuffer {
public:
    explicit ShmRingBuffer(int semid, Auth auth=slave, std::size_t cap=20, char const* path=DEFAULT_PATH);
    ~ShmRingBuffer() {}
...
template <typename T>
ShmRingBuffer<T>::ShmRingBuffer(int semid, Auth auth, std::size_t cap, char const* path) :
    // m_capacity(cap),
    m_hdr(nullptr),
    m_item(nullptr),
    m_semid(semid),
...
template <typename T>
bool ShmRingBuffer<T>::init(std::size_t cap) {
    m_shm_size = sizeof(ShmHandler) + cap * sizeof(T);
    int shm_fd = shm_open(m_shm_path.c_str(), O_CREAT | O_RDWR, S_IRWXU | S_IRWXG);
    if (shm_fd < 0) {
...
template <typename T>
void ShmRingBuffer<T>::push_back(T const& e) {
    P(0);
    memcpy(m_item + m_hdr->_end, &e, sizeof(e));
    m_hdr->_end = (m_hdr->_end + 1) % m_hdr->_capacity; // make sure index is in range [0..._capacity)
    V(1);
}

template <typename T>
T ShmRingBuffer<T>::pop_front() {
    P(1);
    T ans;
...
template <typename T>
void ShmRingBuffer<T>::P(int index) {
    struct sembuf buf;
...
template <typename T>
void ShmRingBuffer<T>::V(int index) {
    struct sembuf buf;
    memset(&buf, 0, sizeof(buf));
    buf.sem_num = index;
    buf.sem_op = 1;
